...
export const RecommendedJobsPage = ({ onNavigateTab }) => {
  ...
  const loadRecommendedJobs = async () => {
    ...
    try {
      console.log(`[RecommendedJobsPage] Querying recommended jobs for candidateId: "${candidateId}", trade: "${trade}", district: "${district}"`);
      ...
      console.log(`[RecommendedJobsPage] Raw response data:`, data);
      if (data?.jobs) {
        data.jobs.forEach(j => {
          console.log(`[RecommendedJobsPage Job Item] title: "${j.title}", match%: ${j.match_percentage}%, matched_skills: [${(j.matched_skills || []).join(', ')}], missing_skills: [${(j.missing_skills || []).join(', ')}]`);
        });
      }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 font-sans">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h1 className="font-display text-xl font-bold tracking-tight text-white">Recommended Industry Job Vacancies</h1>
              <p className="text-xs text-slate-300">
                Ranked by explainable skill-overlap between candidate verified competencies and employer vacancy standards.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            {import.meta.env.DEV && (
              <div className="bg-white/10 p-2 rounded-lg border border-white/15 text-xs">
                <span className="text-[10px] text-blue-200 block mb-1 font-semibold uppercase">
                ...
                >
                  {defaultCandId && !['cand-01', 'cand-low', 'cand-05'].includes(defaultCandId) && (
                    <option value={defaultCandId}>Active Trainee ({defaultCandId})</option>
                  )}
                  <option value="cand-01">Rahul Sharma (cand-01 • Post-Assessment: 85% / 65%)</option>
                  <option value="cand-low">Arjun Rao (cand-low • Low Assessment: 35% / 30%)</option>
                  <option value="cand-05">Vikas Shinde (cand-05 • Prerequisite Pending)</option>
                </select>
              </div>
            )}
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 btn-sid-primary text-xs py-2 px-4 shadow-sm"
            >
        ...
        <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white border border-slate-300 font-bold text-[#0B3D6B] rounded-lg p-2 focus:ring-1 focus:ring-[#0B3D6B]"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white border border-slate-300 font-bold text-slate-800 rounded-lg p-2 focus:ring-1 focus:ring-[#0B3D6B]"
            >
      ...
      {notification && (
        <div className={`p-4 rounded-xl border text-xs flex items-center justify-between gap-3 shadow-xs ${
          ...
          {notification.type === 'success' && (
            <button
              onClick={() => onNavigateTab && onNavigateTab('applications')}
              className="text-xs font-bold text-emerald-800 underline hover:text-emerald-950 flex-shrink-0"
            >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 sm:p-12 text-center max-w-2xl mx-auto space-y-6">
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-bold uppercase tracking-wider bg-[#FDEEE0] text-[#D2691E] px-3 py-1 rounded-full border border-[#F8D3B8] font-mono">
              Prerequisite Required • Assessment
            </span>
            <h2 className="font-display text-xl font-bold text-slate-900">
              Complete your assessment to unlock job recommendations
            </h2>
            <p className="text-xs text-slate-600 max-w-md mx-auto leading-relaxed">
              Recommended jobs are matched against your verified competency scores (threshold score ≥ 60%). Complete your post-training skill evaluation to unlock personalized job matches.
            </p>
          </div>

          {/* Three-step visual road map */}
          <div className="p-5 rounded-xl bg-[#F8F9FA] border border-slate-200 text-left max-w-lg mx-auto space-y-3 text-xs">
            ...
            <div className="space-y-2 text-slate-600">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 font-bold flex items-center justify-center text-[10px]">✓</span>
                <span className="line-through text-slate-400">Step 1: Onboarding & Workshop Training Registration</span>
              </div>
              <div className="flex items-center gap-2 font-bold text-[#D2691E]">
                <span className="w-5 h-5 rounded-full bg-[#FDEEE0] text-[#D2691E] border border-[#F8D3B8] flex items-center justify-center text-[10px]">2</span>
                <span>Step 2: Complete Post-Training Skill Assessment (Action Needed)</span>
              </div>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <button
              ...
              className="text-xs text-slate-500 hover:text-[#0B3D6B] underline font-medium"
            >
        ...
        /* CANDIDATE HAS COMPLETED ASSESSMENT: SHOW RANKED EXPLAINABLE TABLE */
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-2 border-b border-slate-200">
            <div>
              <span className="sid-eyebrow">Direct Matching</span>
              ...
              <p className="text-xs text-slate-500">
                Ordered by competency overlap percentage. Expand any row to inspect matched competencies.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-300 px-3 py-1 rounded-md font-mono">
              ...
              <table className="table-govt">
                <thead>
                  <tr>
                    ...
                    <th className="w-40 text-center">Competency Match</th>
                      ...
                      <React.Fragment key={job.job_id || job.id}>
                        <tr className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="text-xs text-slate-700">
                            ...
                            <span className="text-[10px] font-bold text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded mt-0.5 inline-block">
                              {job.source || 'NCS'}
                            </span>
                          </td>
                          ...
                          <td className="text-center">
                            <div className={`inline-flex items-center gap-1 font-mono text-xs font-bold px-2.5 py-1 rounded-md border ${
                              matchPct >= 60 
                            ...
                            <button
                              ...
                              className="text-[11px] text-[#0B3D6B] hover:underline block mx-auto mt-1 font-semibold"
                            >
                          ...
                          <td className="text-right">
                            {isApplied ? (
                              <span className="bg-emerald-50 text-emerald-800 border border-emerald-300 text-xs font-bold py-1.5 px-3 rounded-md inline-flex items-center gap-1">
                              ...
                              <button
                                ...
                                disabled={isApplying}
                                className="btn-sid-primary text-xs py-1.5 px-3.5 font-bold shadow-2xs"
                              >
                          ...
                          <tr className="bg-slate-50/90 border-b border-slate-200">
                            <td colSpan={6} className="p-4">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                                <div className="p-3 rounded-lg bg-white border border-emerald-200 space-y-2 shadow-2xs">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {matchedSkills.length > 0 ? (
                                      matchedSkills.map((sk, idx) => (
                                        <span key={idx} className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200">
                                          <Check className="w-2.5 h-2.5 text-emerald-600" />
                                          <span>{sk}</span>
                                        </span>
                                      ))
                                    ) : (
                                      <p className="text-[11px] text-slate-400 italic">No direct competency overlap recorded</p>
                                    )}
                                  </div>
                                </div>

                                <div className="p-3 rounded-lg bg-white border border-[#F8D3B8] space-y-2 shadow-2xs">
                                  <div className="flex items-center gap-1.5 text-[#D2691E] font-bold text-[11px]">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {missingSkills.length > 0 ? (
                                      missingSkills.map((sk, idx) => (
                                        <span key={idx} className="inline-flex items-center gap-1 text-[10px] font-bold text-[#D2691E] bg-[#FDEEE0] px-2.5 py-1 rounded-md border border-[#F8D3B8]">
                                      ...
                                      <p className="text-[11px] text-emerald-700 font-bold flex items-center gap-1">
                                        ...
                                        <span>Full competency overlap achieved!</span>
                                      </p>
                                    )}
                                  </div>
                                </div>
      ...
      {/* ── SkillSetuu — AI Skill Guidance Panel ──────────────────── */}
      <div className="mt-6">
        <SkillAssistant
