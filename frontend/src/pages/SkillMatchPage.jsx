...
export const SkillMatchPage = ({ onNavigateTab }) => {
  ...
  const loadGapAnalysis = async () => {
    setLoading(true);
    try {
      console.log(`[SkillMatchPage] Loading gap analysis for candidateId: "${candidateId}", trade: "${trade}", district: "${district}"`);
      ...
      console.log(`[SkillMatchPage] Raw gapData response:`, data);
      if (data?.skillGapVector) {
        data.skillGapVector.forEach(sk => {
          console.log(`[SkillMatchPage Gap Vector Item] skill: "${sk.skill_name}", hasPostScore: ${sk.hasPostScore}, raw candidateScore: ${sk.candidateScore}%`);
        });
      }
      setGapData(data);
    } catch (err) {
    ...
    <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8 font-sans">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className="text-xs text-blue-200 font-mono">Real-Time Labor Market Index</span>
              </div>
              <h1 className="font-display text-xl font-bold tracking-tight text-white">
                Candidate Competency vs. Market Demand Analysis
              </h1>
              <p className="text-xs text-slate-300">
                Evaluating candidate post-assessment competency against local employer vacancy frequencies.
              </p>
            </div>
          </div>

          <div className="bg-white/10 px-4 py-2 rounded-lg border border-white/15 text-xs self-start md:self-auto flex-shrink-0">
            <p className="text-[10px] uppercase font-bold text-emerald-300">Analyzed Postings ({district})</p>
            <p className="text-lg font-bold text-white font-mono">{totalAnalyzed} Live Positions</p>
          </div>
        </div>
      </div>

      {/* Filter Control Bar */}
      <div className="bg-white p-5 sm:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
        <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            <Filter className="w-4 h-4 text-[#0B3D6B]" />
            <span className="font-bold text-slate-700 uppercase tracking-wider text-[11px]">Target Trade:</span>
            <select
              value={trade}
              onChange={(e) => setTrade(e.target.value)}
              className="bg-white border border-slate-300 font-bold text-[#0B3D6B] rounded-lg p-2 focus:ring-1 focus:ring-[#0B3D6B] cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-700 uppercase tracking-wider text-[11px]">District:</span>
            <select
              value={district}
              onChange={(e) => setDistrict(e.target.value)}
              className="bg-white border border-slate-300 font-bold text-slate-800 rounded-lg p-2 cursor-pointer focus:ring-1 focus:ring-[#0B3D6B]"
            >
        ...
        <button 
          ...
          className="btn-sid-secondary text-xs py-2 px-4 flex items-center gap-1.5 self-end sm:self-auto"
        >
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 sm:p-7 space-y-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
          <div>
            <span className="sid-eyebrow">Gap Visualizer</span>
            <h2 className="font-display text-base font-bold text-slate-900 mt-0.5 flex items-center gap-2">
              <BarChart2 className="w-4 h-4 text-[#0B3D6B]" />
              <span>Skill Match & Demand Gap Bar Chart</span>
            </h2>
            <p className="text-xs text-slate-500">
              Candidate Post-Assessment Competency (%) vs. Industry Demand Frequency (%)
            </p>
          </div>
          ...
          {hasCompletedPostAssessment && (
            <div className="flex items-center gap-4 text-xs font-bold self-start sm:self-auto">
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm bg-[#0B3D6B] inline-block" />
                <span className="text-slate-700 font-sans">Post Score (%)</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm bg-[#D2691E] inline-block" />
                <span className="text-[#D2691E] font-sans">Market Demand (%)</span>
              </div>
            </div>
          )}
        </div>
        ...
        {loading ? (
          <div className="py-16 text-center text-slate-400 text-xs space-y-2">
            <div className="w-9 h-9 rounded-full border-2 border-[#D2691E] border-t-transparent animate-spin mx-auto" />
            <p className="font-medium">Evaluating candidate skill vectors against {totalDataset} job postings...</p>
          </div>
        ) : !hasCompletedPostAssessment ? (
          /* EMPTY STATE WITH TWO-TONE ILLUSTRATION */
          <div className="py-10 px-4 text-center max-w-xl mx-auto space-y-4">
            <div className="flex justify-center">
              <EmptyAssessmentIllustration className="w-20 h-20" />
            </div>

            <div className="space-y-1.5">
              <span className="text-[10px] font-bold uppercase tracking-widest bg-[#FDEEE0] text-[#D2691E] px-3 py-1 rounded border border-[#F8D3B8] font-mono">
                POST-ASSESSMENT REQUIRED
              </span>
              <h3 className="font-display text-lg font-bold text-[#0B3D6B] pt-1">
                Complete your post-training assessment to unlock gap analysis
              </h3>
              ...
              return (
                <div 
                  ...
                  className={`p-4 rounded-xl border transition-all space-y-3 ${
                    hasPost ? 'bg-white border-slate-200 shadow-2xs' : 'bg-slate-50/60 border-slate-200'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                    <div>
                      <span className="text-[10px] font-mono font-bold text-[#0B3D6B] bg-[#E8ECFB] px-2 py-0.5 rounded border border-[#D1DBF7]">
                        {sk.category}
                      </span>
                      <h3 className={`text-sm font-bold mt-1 ${hasPost ? 'text-slate-900' : 'text-slate-500'}`}>
                        {sk.skill_name}
                      </h3>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {hasPost ? (
                        <span className={`text-xs font-bold font-mono px-2.5 py-0.5 rounded-md border flex items-center gap-1 ${
                          (candScore >= 60 && gap >= 0)
                            ? 'bg-emerald-50 text-emerald-800 border-emerald-300' 
                            : 'bg-[#FDEEE0] text-[#D2691E] border-[#F8D3B8]'
                        }`}>
                          {(candScore >= 60 && gap >= 0) ? <ArrowUpRight className="w-3.5 h-3.5 text-emerald-600" /> : <ArrowDownRight className="w-3.5 h-3.5 text-[#D2691E]" />}
                          <span>Gap Score: {gap > 0 ? `+${gap}%` : `${gap}%`}</span>
                        </span>
                      ) : (
                        <span className="text-[11px] font-medium px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 border border-slate-200 font-mono">
                          Not assessed yet
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-2 pt-1">
                    ...
                    <div>
                      ...
                      <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                        {hasPost ? (
                          <div 
                            className="bg-[#0B3D6B] h-full rounded-full transition-all duration-700 ease-out"
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] font-semibold text-slate-700 mb-1">
                        ...
                        <span className={`font-mono font-bold ${hasPost ? 'text-[#D2691E]' : 'text-slate-500'}`}>
                          {demandFreq}%
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-700 ease-out ${
                            hasPost ? 'bg-[#D2691E]' : 'bg-slate-300'
                          }`}
      ...
      {gapData?.recommendations && gapData.recommendations.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 sm:p-7 space-y-5">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              <span className="sid-eyebrow">Local Priorities</span>
              <h2 className="font-display text-base font-bold text-slate-900 mt-0.5 flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-[#0B3D6B] bg-[#E8ECFB] px-2.5 py-1 rounded border border-[#D1DBF7]">
              Explainable Scoring
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {gapData.recommendations.map((rec, idx) => (
              <div 
                key={idx} 
                className="p-5 rounded-xl border border-slate-200 bg-white hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col justify-between space-y-3 shadow-2xs"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-[#D2691E] bg-[#FDEEE0] border border-[#F8D3B8] px-2 py-0.5 rounded font-mono">
                      {rec.skill_demand_percentage}% Market Demand
                    </span>
                  </div>

                  <h3 className="font-display text-sm font-bold text-slate-900">{rec.skill_name}</h3>

                  <div className="p-3 rounded-lg bg-[#FDEEE0] border border-[#F8D3B8] text-[11px] text-slate-800 font-medium space-y-1">
                    <p className="font-bold flex items-center gap-1 text-[#D2691E]">
                      <TrendingUp className="w-3.5 h-3.5 text-[#D2691E] flex-shrink-0" />
                      <span>Upskill Rationale:</span>
                    </p>
                    <p className="leading-snug text-slate-700">{rec.reason}</p>
                  </div>
                </div>

                <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
                  <span className="text-slate-500">
                    Status: <strong className={rec.achieved ? 'text-emerald-700' : 'text-[#D2691E]'}>{rec.achieved ? 'Achieved (≥60%)' : 'Needs Upskilling'}</strong>
                  </span>
                  <span className="font-mono font-bold text-[#0B3D6B]">{(rec.post_score !== undefined && rec.post_score !== null) ? rec.post_score : (rec.current_score ?? 0)}% Score</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 sm:p-7 space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
          <div>
            <span className="sid-eyebrow">Local Employment Matches</span>
            <h2 className="font-display text-base font-bold text-slate-900 mt-0.5 flex items-center gap-2">
              <Briefcase className="w-4 h-4 text-[#0B3D6B]" />
              <span>Explainable Job Matches ({trade} • {district})</span>
            </h2>
            <p className="text-xs text-slate-500">Skills you have vs. skills you're missing for each local posting</p>
          </div>
          <div className="flex items-center gap-2">
          ...
          {matchedJobs.map((job) => {
            ...
            return (
              <div 
                ...
                className="p-5 rounded-xl border border-slate-200 hover:border-[#D2691E]/40 hover:shadow-md bg-white space-y-3.5 transition-all shadow-2xs flex flex-col justify-between"
              >
                <div className="space-y-3">
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      ...
                      <h3 className="font-display text-sm font-bold text-slate-900 mt-1">{job.title}</h3>
                      <p className="text-xs text-slate-600">{job.company_name || job.company} • <span className="font-semibold text-slate-800">{job.district || district}</span></p>
                    </div>

                    {/* Overall Match Percentage Badge */}
                    <div className="flex flex-col items-end flex-shrink-0">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full font-mono shadow-2xs flex items-center gap-1 ${
                        matchPct === 100 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-xs">
                    ...
                    {/* Skills you're missing (saffron/peach) */}
                    <div className="p-2.5 rounded-lg bg-[#FDEEE0]/70 border border-[#F8D3B8] space-y-1.5">
                      <div className="flex items-center gap-1 text-[#D2691E] text-[11px] font-bold">
                        <AlertTriangle className="w-3.5 h-3.5 text-[#D2691E] flex-shrink-0" />
                      ...
                      {missingSkills.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {missingSkills.map((sk, idx) => (
                            <span 
                              key={idx} 
                              className="inline-flex items-center gap-1 text-[10px] font-bold text-[#D2691E] bg-white px-2 py-0.5 rounded border border-[#F8D3B8] shadow-2xs"
                            >
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-100">
                  ...
                  <button className="btn-sid-primary text-[11px] py-1.5 px-3.5 flex items-center gap-1 shadow-2xs">
                    <span>Apply Now</span>
